...
async def get_upload_url(
    ...
    try:
        result = supabase.table("projects").select("id").eq("project_id", project_id).eq("clerk_id", clerk_id).execute()
        if not result.data:
            raise HTTPException(status_code=400, detail="Project not found")
        file_extention = file_request.file_name.split('.')[-1] if '.' in file_request.file_name else ''
        unique_id = str(uuid.uuid4())
        s3_key = f"projects/{project_id}/documents/{unique_id}.{file_extention}"
